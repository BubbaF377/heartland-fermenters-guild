...
- **Resolved:** Devlore's auto-doc-sync (`devlore.yml`'s `sync-test-plan`/`sync-user-manual`/`sync-visualizer`/`draft-log-entry` jobs, plus `devlore-release.yml`'s release-notes job) was broken as of 2026-08-15 because this repo is public while its reusable workflows lived in the private `devlore` repo, which GitHub disallows. Fixed by making `devlore`'s reusable workflows public; the sync jobs ran successfully afterward.
